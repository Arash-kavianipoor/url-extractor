...
async function startServer() {
  ...
  // Explicit handler for extension zip download with strict Content-Length and MIME type
  const serveExtensionZip = (req: express.Request, res: express.Response) => {
    const zipPath = path.resolve(process.cwd(), 'public/web-scraper-extension.zip');
    if (fs.existsSync(zipPath)) {
      const stat = fs.statSync(zipPath);
      res.setHeader('Content-Type', 'application/zip');
      res.setHeader('Content-Length', stat.size.toString());
      res.setHeader('Content-Disposition', 'attachment; filename="web-scraper-pro-extension.zip"');
  ...
  app.get('/api/download-extension', serveExtensionZip);
  app.get('/web-scraper-extension.zip', serveExtensionZip);
